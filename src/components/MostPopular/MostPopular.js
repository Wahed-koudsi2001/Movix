...
export default function MostPopular() {
    ...
    const { data } = useFetchMainData(`https://api.themoviedb.org/3/${selected}/popular`, true);
    ...
    return (
        <div className="container mx-auto m-[100px]">
            <div className="flex flex-col md:flex-row items-center justify-center md:justify-between mb-5 md:px-2">
                <h2 className="text-white mb-5 md:mb-0 text-3xl">Must Popular</h2>
            ...
            <div className="glider-container-wrapper">
                ...
                >
                    {data?.map((card) =>
                        <Link to={selected === "movie" ? `movie/${card.id}` : `tv/${card.id}`} className='glider-card m-3' key={card.id}>
