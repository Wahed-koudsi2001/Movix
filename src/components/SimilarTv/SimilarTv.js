...
export default function SimilarTv() {
    const { id } = useParams();
    const { data: similar } = useFetchMainData(`https://api.themoviedb.org/3/tv/${id}/similar`, true);
        ...
        <div>
            {similar?.length > 0 &&
                <div className="glider-container-wrapper mt-[80px]">
                    <h1 className="mb-2 text-[24px]">Similar Movies</h1>
                    <Glider
                    ...
                    >
                        {similar?.map((card) =>
                            <Link to={`/tv/${card.id}`} key={`movie-${card.id}`} className='glider-card m-3 wahed'>
                                ...
                                <div className='mt-5'>
                                    <h6 className='text-[#fff] h-[40px]'>{card.title || card.original_name}</h6>
