...
export default function RecommendationsTv() {

    const { id } = useParams();
    const { data: recommendations } = useFetchMainData(`https://api.themoviedb.org/3/tv/${id}/recommendations`, true);
        ...
        <div>
            {recommendations?.length > 0 ?
                <div className="glider-container-wrapper mt-[80px]">
                    <h1 className="mb-2 text-[24px]">Recommendations</h1>
                    <Glider
                        ...
                        {recommendations?.map((card) =>
                            <Link to={`/tv/${card.id}`} key={`movie-${card.id}`} className='glider-card m-3 wahed'>
                                ...
                                <div className='mt-5'>
                                    <h6 className='text-[#fff] h-[40px]'>{card.title || card.name}</h6>
