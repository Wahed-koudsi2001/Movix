...
export default function SimilarMovies() {
    ...
    const { id } = useParams();
    const { data: similar } = useFetchMainData(`https://api.themoviedb.org/3/movie/${id}/similar`, true);
        ...
        <>
            {similar?.length > 0 ?
                <div className="glider-container-wrapper mt-[80px]">
                    <h1 className="mb-5 text-[24px]">Similar Movies</h1>
                    <Glider
                    ...
                    >
                        {similar?.map((card) =>
                            <Link to={`/movie/${card.id}`} key={`movie-${card.id}`} className='glider-card m-3 wahed'>
                                ...
                                <div className='mt-5'>
                                    <h6 className='text-[#fff] h-[40px]'>{card.title.slice(0, 15)}</h6>
