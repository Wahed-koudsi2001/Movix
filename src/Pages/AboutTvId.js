...
import ImagesForEachId from "../components/ImagesForEachId/ImagesForEachId";
import VideosTv from "../components/VideosTv/VideosTv";
import SimilarTv from "../components/SimilarTv/SimilarTv";
import RecommendationsTv from "../components/RecommendationsTv/RecommendationsTv";
...
export default function AboutMovieId() {
    ...
    const { data, error } = useFetchMainData(`https://api.themoviedb.org/3/tv/${id}`, false);
        ...
        <div className="text-[#fff] relative">
            ...
            <div className="flex flex-col md:flex-row items-center gap-[50px] relative pt-[80px] z-40 container mx-auto px-[20px]">
                ...
                <div className='w-full lg:w-7/12 mt-[100px] lg:mt-0'>
                    <h1 className="text-[24px] md:text-[34px] m-0">{data.name}({data.first_air_date.slice(0, 4)})</h1>
                    ...
                    <div>
                        <div className="flex flex-col gap-5 justify-between border-b-[0.1px] border-slate-400 pb-[15px] mb-[15px]">
                            ...
                            <p className='font-bold'>Release Date: <span className='ms-[10px] font-light opacity-[0.5]'>{data.first_air_date}</span></p>
                        </div>
                    </div>
                </div>
            </div>
